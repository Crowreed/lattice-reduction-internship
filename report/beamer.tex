...
\title{The \( \mathrm{LLL} \) Algorithm: Lattice Basis Reduction and applications to Approximate Shortest Vector Problem}
\author{Lucas Petit}
\date{May 26, 2025}
                ...
                \begin{smallalgo}{LLL}{algo:LLL_MCA}
                    ...
                    \tikz[remember picture, baseline=(gs.base)]{
                        \node[inner sep=1pt] (gs) {
                            \( (G^*, U) \leftarrow \) \textsc{Gram-Schmidt} \( G \)\;
                        };
                    }
...
%    \textbf{Pause.}\\[1em]
%    \normalsize
%    Let’s try out the \textsc{LLL} algorithm in \texttt{SageMath}\\
    ...
    \begin{frame}{LLL Complexity}    
        \uncover<2->
        {
            \begin{smallalgo}{LLL}{algo:LLL_MCA}
                ...
                \KwOut{An LLL-reduced basis \( G = (\\bg_1, \ldots, \\bg_n) \)}
                \( G \leftarrow copy(B)\)\;
                \tikz[remember picture, baseline=(gs.base)]{\node[inner sep=1pt] (gs){\( (G^*, U) \leftarrow \) \textsc{Gram-Schmidt} \( G \)\;};}
